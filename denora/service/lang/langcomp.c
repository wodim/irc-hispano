 ...
 *
 * $Id: langcomp.c 386 2011-01-04 17:16:43Z hal9000 $
 *
 */
...
char *denora_getline(FILE * f);
int stringnum(const char *name);
int read_index_file(void);
    ...
    while (maxerr > 0 && (line = denora_getline(in)) != NULL) {
        if (*line == '\t') {
            ...
            if ((curstring = stringnum(line)) < 0) {
                ...
                fprintf(stderr,
                        "%s:%d: Duplicate occurrence of string `%s'\n",
                        filename, linenum, line);
                retval = 1;
                maxerr--;
            } else {
