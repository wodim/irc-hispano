 ...
 *
 * $Id: html_top10chan.c 386 2011-01-04 17:16:43Z hal9000 $
 *
 */
/*************************************************************************/
...
int DenoraInit(int argc, char **argv)
{
    ...
    if (status != MOD_ERR_OK) {
        alog(LOG_NORMAL,
             "Error Occurried setting message for !TOP10CHAN! [%d][%s]",
    ...
    if (status != MOD_ERR_OK) {
        alog(LOG_NORMAL,
             "Error Occurried setting message for !TOP10CHANEVERTITLE! [%d][%s]",
             status, ModuleGetErrStr(status));
    }
    ...
    if (status != MOD_ERR_OK) {
        alog(LOG_NORMAL,
             "Error Occurried setting message for !POPULARCHANTITLE! [%d][%s]",
             status, ModuleGetErrStr(status));
    }
