 ...
 *
 * $Id: html_map.c 386 2011-01-04 17:16:43Z hal9000 $
 *
 */
/*************************************************************************/
...
int DenoraInit(int argc, char **argv)
{
    ...
    moduleAddVersion("$Id: html_map.c 386 2011-01-04 17:16:43Z hal9000 $");
    moduleSetType(CORE);
    ...
    if (status != MOD_ERR_OK) {
        alog(LOG_NORMAL,
             "Error Occurried setting message for !MAP! [%d][%s]", status,
    ...
    if (status != MOD_ERR_OK) {
        alog(LOG_NORMAL,
             "Error Occurried setting message for !NETMAPTITLE! [%d][%s]",
