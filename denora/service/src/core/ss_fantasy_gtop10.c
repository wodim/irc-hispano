 ...
 * 
 * $Id: ss_fantasy_gtop10.c 386 2011-01-04 17:16:43Z hal9000 $
 *
 */
...
int do_fantasy(int argc, char **argv)
{
    ...
    if (stricmp(argv[0], "gtop10") == 0) {
        u = finduser(argv[1]);
        chan = rdb_escape(argv[2]);
        cs = find_cs(argv[2]);
        rdb_query
            (QUERY_HIGH,
             "SELECT * FROM %s WHERE chan=\'global\' AND type=0 ORDER BY letters DESC LIMIT 10;",
             UStatsTable);
#ifdef USE_MYSQL
        mysql_res = mysql_store_result(mysql);
        if (mysql_num_rows(mysql_res) > 0) {
            if (cs->flags & CS_NOTICE) {
                notice_lang(s_StatServ, u, STATS_TOP10_NETWORK, argv[2]);
            } else {
