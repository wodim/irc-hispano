 ...
 *
 * $Id: html_server.c 386 2011-01-04 17:16:43Z hal9000 $
 *
 */
/*************************************************************************/
...
int DenoraInit(int argc, char **argv)
{
    ...
    if (status != MOD_ERR_OK) {
        alog(LOG_NORMAL,
             "Error Occurried setting message for !SRVLIST! [%d][%s]",
    ...
    if (status != MOD_ERR_OK) {
        alog(LOG_NORMAL,
             "Error Occurried setting message for !SERVERLISTTILE! [%d][%s]",
             status, ModuleGetErrStr(status));
    }
