...
//
#define RCSID "$Id: PXM3Com812.cc,v 1.4 2006/09/10 22:16:39 spale Exp $"

// This class scans for Open Wingate or 3com812 or Cisco (with "cisco" password).
...
#define R3COM812_PORT     WINGATE_PORT
...
uint32_t PXM3Com812::sConnCount = 0;
uint32_t PXM3Com812::sProxyCount1 = 0;
uint32_t PXM3Com812::sProxyCount2 = 0;
uint32_t PXM3Com812::sProxyCount3 = 0;

PXM3Com812::PXM3Com812(PXScan *inScan)
  : PXScanModule(inScan), mStream(NULL), mCiscoRequest(false), mLinesTry(0)
  {
  }

PXM3Com812::~PXM3Com812()
  {
  }

void
PXM3Com812::InitModule()
  {
  ...
  RegisterPXM(R3COM812_SHORTNAME, R3COM812_PORT, &sConnCount, &sProxyCount3);
  }

bool
PXM3Com812::StartScan()
  {
...
void
PXM3Com812::SendCiscoRequest(peak_stream s)
  {
...
void
PXM3Com812::ProcessEvent(peak_stream s, int type)
  {
  char *line;
  char winReadBuf[64];
  int err;
      ...
      if (!mCiscoRequest)
        ...
        if (peak_stream_read(s, winReadBuf, sizeof(winReadBuf)) > 0)
          ...
          if (!strncmp(winReadBuf, "WinGate>", 8)
            ...
            sProxyCount1++;
#if 0
            clog << "PXM3Com812::ProcessEvent: proxy found: "
                 << winReadBuf << endl;
#endif
            peak_release(s);
          ...
          if (!strncmp(winReadBuf, "login:", 6)
              || !strncmp(winReadBuf, "password:", 9))
            {
            sProxyCount3++;
#if 0
            clog << "PXM3Com812::ProcessEvent: proxy found: "
...
#endif
            peak_release(s);
            this->ProxyFound(OPAS_PROXY_TYPE_3COM812, R3COM812_PORT,
                             R3COM812_DESCR);
            return; /* we are done */
            }
          }
...
void
PXM3Com812::EventCallback(peak_stream s, int type, void *context)
  {
  PXM3Com812 *pxm = reinterpret_cast<PXM3Com812*>(context);
